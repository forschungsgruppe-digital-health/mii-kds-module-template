...
```
{%raw%}{% include StructureDefinition-example-patient-dict.xhtml %}{%endraw%}
```
...
`{%raw%}{% fragment %}{%endraw%}` renders an instance held in this guide, and can
...
```
{%raw%}{% fragment Patient/ExamplePatientInstance JSON BASE:name %}{%endraw%}
```
...
```
{%raw%}{% sql select Name, Description from Resources order by Name %}{%endraw%}
```
...
rendering (`link`, `markdown`, `canonical`, `resource`, …), and
`{%raw%}{% sqlToData %}{%endraw%}` puts the rows into a Liquid variable instead
of rendering a table, so you can lay them out yourself.
