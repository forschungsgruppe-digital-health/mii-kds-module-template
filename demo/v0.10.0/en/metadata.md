# Metadata Overview - MII Implementation Guide Module Template v2027.0.0-template.0.10.0
...
The approach is preliminary: it records the CRMI-based metadata currently used and may be refined as CRMI matures, as the MII publication process evolves, and as FAIR assessment of FHIR implementation guides becomes more concrete.
...
CRMI organises artifact management into lifecycle phases and supporting concerns. This guide does not implement every CRMI capability; it applies the parts that are directly useful for publishing an MII module.
...
The following CRMI-related metadata is set in [`sushi-config.yaml`](https://github.com/medizininformatik-initiative/mii-kds-module-template/blob/main/sushi-config.yaml) and ends up in the generated `ImplementationGuide` resource.
...
| | | | |
| :--- | :--- | :--- | :--- |
| F1 | RDA-F1-01M | Metadata is identified by a persistent identifier | Canonical`url`values, the package id`de.medizininformatikinitiative.kerndatensatz.template`, the package version and`package-source`. Persistence depends on publication governance. |
