...
7. **Add content.** Replace the example profile in `input/fsh/` with your own
   ...
   `input/translations/de/` in step with them. When you add, rename or remove a
   page, update all four places that name it: the `pages:` tree in
   `sushi-config.yaml`, both `menu.xml` files, and the IG-level catalogue from
   step 4.
