...
| `convention-check` red on the release branch: M9 | Optional (0..1) pages still carry their `OPTIONAL-PAGE` banner, or menu labels still say `(optional)` | Decide each one — keep (delete the banner + the menu-label suffix in both languages) or remove per [remove-an-optional-page.md](remove-an-optional-page.md) (checklist: [optional-pages.md](../optional-pages.md)) |
