...
| --- | --- | --- | --- | --- | --- |
| `ig-publisher.yml` | push to any branch except `main`/`gh-pages`/`fsh-generated`; `workflow_dispatch` | Builds the IG (SUSHI + IG Publisher) and deploys a preview. **On this template repo only**, the self-check build also demonstrates the version comparison: the last dev preview's package is seeded as `2027.0.0-draft.0` and the preview publishes `comparison-v2027.0.0-draft.0/` — on `dev` a self-comparison (what a no-changes release looks like), on a feature branch the real delta against `dev`. A created module keeps `version-comparison` commented out until its second release | `gh-pages/branches/<branch>/` + PR comment | `ENABLE_PREVIEW` (ON) | no |
